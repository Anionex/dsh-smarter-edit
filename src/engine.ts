...
async function resolveTarget(cwd: string, input: string): Promise<{ relativePath: string; absolutePath: string }> {
...
function fingerprint(info: Awaited<ReturnType<typeof stat>>): string {
...
async function readSnapshot(absolutePath: string, signal?: AbortSignal): Promise<FileSnapshot> {
...
export function mutationPaths(parsed: ParsedPatch): string[] {
  return parsed.operations.flatMap(operationPaths)
}

async function buildPlan(options: ApplyPatchOptions): Promise<PatchPlan> {
  const parsed = parsePatch(options.patch)
  const cwd = resolve(options.cwd)

  const targets = new Map<string, PlannedTarget>()
  const files: AppliedFile[] = []
  const targetFor = async (rawPath: string): Promise<PlannedTarget> => {
...
async function validateTargets(plan: PatchPlan, options: ApplyPatchOptions): Promise<void> {
  if (options.validatePath === undefined) return
  for (const target of plan.targets) {
    await options.validatePath(target.rawPath, target.absolutePath)
  }
}

async function createMissingParents(directory: string, created: string[]): Promise<void> {
...
async function stageTargets(plan: PatchPlan, createdDirectories: string[], options: ApplyPatchOptions): Promise<void> {
  const transactionId = randomUUID()
  for (const [index, target] of plan.targets.entries()) {
    if (target.newContent === null) continue
    throwIfAborted(options.signal)
...
async function verifyOriginals(plan: PatchPlan, signal?: AbortSignal): Promise<void> {
  for (const target of plan.targets) {
...
async function verifyCapturedBackup(target: PlannedTarget, signal?: AbortSignal): Promise<void> {
  const captured = await readSnapshot(target.backupPath as string, signal)
...
async function verifyFinals(plan: PatchPlan, signal?: AbortSignal): Promise<void> {
  for (const target of plan.targets) {
    const current = await readSnapshot(target.absolutePath, signal)
    if (target.newContent === null) {
      if (current.exists) throw new PatchError(`apply_patch: deletion verification failed: ${target.relativePath}`, 'PATCH_IO')
...
async function removeCreatedDirectories(paths: readonly string[]): Promise<void> {
  for (const path of [...paths].reverse()) {
    try {
      ...
      if (!isMissing(error) && (error as { code?: unknown }).code !== 'ENOTEMPTY') throw error
    }
  }
}

async function rollback(plan: PatchPlan, createdDirectories: readonly string[]): Promise<void> {
  ...
  for (const target of plan.targets) {
    try {
      ...
      if (!target.backedUp) await removeIfPresent(target.backupPath)
    } catch (error) {
...
async function commit(plan: PatchPlan, options: ApplyPatchOptions): Promise<void> {
  ...
  try {
    ...
    for (const [index, target] of plan.targets.entries()) {
      ...
      if (target.newContent !== null) {
        await link(target.stagePath as string, target.absolutePath)
        target.published = true
        await unlink(target.stagePath as string)
    ...
    try {
      ...
      throw new PatchError(
        `apply_patch: transaction failed (${patchErrorMessage(error)}); ${patchErrorMessage(rollbackError)}`,
...
function canonicalDiff(plan: PatchPlan): string {
...
function codexSummary(files: readonly AppliedFile[]): string {
  ...
  for (const file of files.filter(candidate => candidate.action === 'update' || candidate.action === 'move')) lines.push(`M ${file.path}`)
  for (const file of files.filter(candidate => candidate.action === 'delete')) lines.push(`D ${file.path}`)
  return lines.join('\n')
}

/** Parse, preflight, stage, atomically publish, verify, and summarize one patch. */
export async function applyPatchAtomic(options: ApplyPatchOptions): Promise<ApplyPatchResult> {
