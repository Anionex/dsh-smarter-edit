...
function findWith(lines: readonly string[], pattern: readonly string[], start: number, compare: (left: string, right: string) => boolean): number | undefined {
  const last = lines.length - pattern.length
  for (let index = start; index <= last; index += 1) {
    if (pattern.every((line, offset) => compare(lines[index + offset] as string, line))) return index
  }
  return undefined
}

/** Direct port of Codex apply-patch's seek_sequence in PreserveLineEndings mode. */
export function seekSequence(lines: readonly string[], pattern: readonly string[], start: number, atEnd: boolean): number | undefined {
...
function computeReplacements(lines: readonly string[], path: string, chunks: readonly UpdateChunk[]): Replacement[] {
...
export function applyChunks(content: string, path: string, chunks: readonly UpdateChunk[]): string {
