...
export function unwrapApplyPatchArguments(serialized) {
    ...
    throw new PatchError('dsh-apply-patch: pi-ai did not return the expected raw custom-tool input', 'PATCH_UNSUPPORTED');
}
...
function decodeApplyPatchDelta(delta, state) {
    ...
    if (state.mode === 'unknown') {
        ...
        if (trimmed.startsWith('{')) {
            ...
            throw new PatchError('dsh-apply-patch: model changed its patch-tool streaming envelope', 'PATCH_UNSUPPORTED');
        }
    }
    try {
...
export async function installPiAiFreeformBridge() {
    ...
    if (typeof original !== 'function') {
        throw new PatchError('dsh-apply-patch: this dsh-llm-pi-ai version has no compatible current snapshot boundary', 'PATCH_UNSUPPORTED');
    }
    ...
    if (typeof originalStream !== 'function') {
        ...
        throw new PatchError('dsh-apply-patch: this dsh-llm-pi-ai version has no compatible stream boundary', 'PATCH_UNSUPPORTED');
    }
    ...
    if (Object.hasOwn(prototype, 'prepareCall') && typeof prototype.prepareCall === 'function') {
        ...
        prepareCallWrapper = async function (...args) {
            ...
            if (typeof prepared !== 'object'
                ...
                throw new PatchError('dsh-apply-patch: pi-ai adapter returned an incompatible prepared call', 'PATCH_UNSUPPORTED');
            }
