...
function splitPatchLines(input) {
    ...
    if (original.length >= 4
        ...
        if (rustTrim(inner[0]) === BEGIN_PATCH && rustTrim(inner.at(-1)) === END_PATCH)
            return inner;
        if (rustTrim(inner[0]) !== BEGIN_PATCH)
...
export function parsePatch(input) {
    if (typeof input !== 'string')
        invalidPatch('input must be a string');
    ...
    for (let index = 1; index < lines.length - 1; index += 1) {
        ...
        if (activeMode.kind !== 'update') {
            ...
            if (startOperation(trimmed, lineNumber))
                continue;
            if (activeMode.kind === 'add' && line.startsWith('+')) {
                activeMode.operation.content += `${line.slice(1)}\n`;
                continue;
            }
            invalidHunk(`'${trimmed}' is not a valid hunk header. Valid hunk headers: '${ADD_FILE}{path}', '${DELETE_FILE}{path}', '${UPDATE_FILE}{path}'`, lineNumber);
        }
        const updateLine = rustTrimEnd(line);
        if (startOperation(updateLine, lineNumber))
            continue;
